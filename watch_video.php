...
if (isset($_SESSION['User_Num'])) {
    $user_id = $_SESSION['User_Num']; // Use session variable
} else {
    $user_id = '';
}
...
<section class="edit-comment">
   ...
   <form id="commentForm" action="" method="post" onsubmit="return validateComment()">
       <h3>Post a Comment</h3>

       <!-- Text area for the comment -->
       <textarea id="comment" name="comment" placeholder="Enter your comment" class="box" required></textarea>

       <!-- Display messages from PHP, like validation errors -->
       <?php
       if (isset($message)) {
...
<section class="watch-video">
   ...
   <div class="video-details">
      ...
      <!-- Display PDF link here -->
      <?php if (!empty($fetch_content['pdf'])) { ?>
         <a href="uploaded_files/<?= $fetch_content['pdf']; ?>" target="_blank" class="pdf-link">
            <p style="font-size: 2rem;"> Download Text Book</p>
         </a>
      <?php } ?>
   </div>
   <?php
...
<section class="comments">
   ...
   <form action="" method="post" class="add-comment" onsubmit="return validateComment();">
      <input type="hidden" name="content_id" value="<?= $get_id; ?>">
      <textarea name="comment_box"  id="comment_box" required placeholder="write your comment..." maxlength="1000" cols="30" rows="10"></textarea>
...
<script>
    const bannedWords = ["hate", "violence", "offensive", "racist", "abuse", "fuck", "voetsek", "kill", "cunt", "sifebe", "bitch", "faggot"];

    function validateComment() {
        let comment = document.getElementById('comment_box').value.toLowerCase();
        for (let i = 0; i < bannedWords.length; i++) {
            if (comment.includes(bannedWords[i])) {
                alert("Your comment contains inappropriate language. Please remove it.");
                return false; // Block form submission
            }
        }
        console.log("Form validated"); // Debugging
